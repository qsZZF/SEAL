...
if ischar(lambda_param_input) || isstring(lambda_param_input)
    switch lower(lambda_param_input)
        case 'auto'
            if isempty(Data_orig)
               error('seal_MNE:Data not provided.'); 
            end
        % learn lambda from data under Gaussian distribution    
            ...
            for iter = 1 : Max_iter
                ...
                gamma = (norm(gamma * L_w' / Cov_B * Data_w,'fro') / sqrt(Ntimepoints * trace(L_w' /Cov_B * L_w)));
...
%     lambda_sq_used = lambda_param_input;
else
