    ...
    switch lower(waveformType)
        case 'erp'
            ...
            for i_comp = 1:length(params.components)
                ...
                if isfield(comp,'type') && strcmpi(comp.type, 'gaussian') && ~isfield(comp,'width_s')
                    warning('Gaussian ERP component lacks .width_s, use 0.05s by default');
                    comp.width_s = 0.05;
                end
                if isfield(comp,'type') && strcmpi(comp.type, 'gabor')
                    if ~isfield(comp,'frequency_Hz'), comp.frequency_Hz = 10; warning('Gabor ERP component lacks .frequency_Hz, use 10Hz by default'); end
                    if ~isfield(comp,'std_dev_s'), comp.std_dev_s = 0.05; warning('Gabor ERP component lacks .std_dev_s, use 0.05 by default'); end
                    if ~isfield(comp,'phase_rad'), comp.phase_rad = 0; warning('Gabor ERP component lacks .phase_rad, use 0 by default');end
                end
        ...
        case 'ersp'
            ...
            if ismember(lower(mod_params.type), {'burst', 'invburst'})
                latency_samples = floor((mod_params.latency_s-timeVector(1))*samplingRate) + 1;
        ...
        case 'ar' % Autoregressive model
            ...
            filter_coeffs = ar_coeffs / norm(ar_coeffs); 
